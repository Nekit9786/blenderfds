...
class VIEW3D_HT_header(Header):
    ...
    def draw(self, context):
        ...
        # Replacement of: layout.template_header_3D()
        if obj and obj.mode == 'EDIT':
            layout.operator("object.editmode_toggle", icon='EDITMODE_HLT', text='Edit Mode')
        else:
            layout.operator("object.editmode_toggle", icon='OBJECT_DATAMODE', text='Object Mode')

        layout.prop(view, "viewport_shade", text='', icon_only=True)
        row = layout.row(align=True)
        row.prop(view, "pivot_point", text='', icon_only=True)
        if obj and obj.mode == 'OBJECT':
            row.prop(view, "use_pivot_point_align", text='', icon_only=True)
        row = layout.row(align=True)
        row.prop(view, "show_manipulator", text='', icon_only=True)
        if view.show_manipulator:
            row.prop(view, "transform_manipulators", text='', icon_only=True)
        row.prop(view, "transform_orientation", text='')

        if context.object and context.object.mode == 'OBJECT':
            active_layers = obj and int(''.join(reversed(list(map(str, map(int, (obj.layers)))))),2) or 0
            layout.template_layers(sc, "layers", view, "layers_used", active_layers)
